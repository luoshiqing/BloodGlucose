...
//        self.leftTableview.frame = CGRectMake(0, 0, kScreenWidth - kMainPageDistance, 7 * 7 + 49 * 7 + 112);
...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (needMoveWithTap && (rec.view.frame.origin.x >= 0) && (rec.view.frame.origin.x <= (kScreenWidth - kMainPageDistance)))
    {
        ...
        CGFloat baseCenterY = (kScreenHeight * (((1 - scale) / 2) + scale)) - 60 * (scale / 2);


        self.baseView.center = CGPointMake(recCenterX,baseCenterY);
...
//                [self.delegate CompeleteAnimation];
...
//        self.baseView.transform = CGAffineTransformScale(CGAffineTransformIdentity,1.0,1.0);
        self.baseView.transform = CGAffineTransformMakeScale(1.0, 1.0);
        
        
        self.baseView.center = CGPointMake(kScreenWidth / 2, kScreenHeight - (63 / 2));
...
//    self.baseView.transform = CGAffineTransformScale(CGAffineTransformIdentity,1.0,1.0);
    ...
    self.baseView.center = CGPointMake(kScreenWidth / 2, kScreenHeight - (60 / 2));
...
//    self.baseView.transform = CGAffineTransformScale(CGAffineTransformIdentity,kMainPageScale,kMainPageScale);
    self.baseView.transform = CGAffineTransformMakeScale(kMainPageScale, kMainPageScale);
//    self.baseView.center = CGPointMake(kScreenWidth + kScreenWidth * kMainPageScale / 2.0 - kMainPageDistance, (kScreenHeight * 0.9) - 63 * 0.4);
    
    CGFloat centY = (kScreenHeight * (((1 - kMainPageScale) / 2.0) + kMainPageScale)) - 60 * (kMainPageScale / 2);
    
    self.baseView.center = CGPointMake(kScreenWidth + kScreenWidth * kMainPageScale / 2.0 - kMainPageDistance, centY);
    
    [UIView commitAnimations];
    [self disableTapButton];
    
    [self.delegate OpenCompeleteAnimation];
...
- (void) removeSingleTap
{
    ...
    [self.delegate CloseCompeleteAnimation];
